...
def normalization(planes, norm='bn'):
    # TODO: need to implement the correct batchnorm
    if norm == 'bn':
...
class Conv3d_Block(tf.keras.layers.Layer):
    def __init__(self,num_in,num_out,kernel_size=1,stride=1,g=1,padding=None,norm=None):
        ...
        self.act_fn = tf.nn.ReLU(inplace=True)
        self.conv = tf.keras.layers.Conv3D(num_in, num_out, kernel_size=kernel_size, padding=padding,stride=stride,groups=g,bias=False)

    def call(self, x): # BN + Relu + Conv
        norm = self.act_fn(self.bn(x))
        convolved = self.conv(norm)
        return convolved

class DilatedConv3DBlock(tf.keras.layers.Layer):
    def __init__(self, num_in, num_out, kernel_size=(1,1,1), stride=1, g=1, d=(1,1,1), norm=None):
        ...
        # Factors in the dilation (dd), which has gaps of 1.
        padding = tuple(
        ...
        self.bn = normalization(num_in, norm=norm)
        self.act_fn = tf.nn.ReLU(inplace=True)
        self.conv = tf.keras.layers.Conv3D(num_in,num_out,kernel_size=kernel_size,padding=padding,stride=stride,dilation=d,group=g,bias=False)
...
class MFunit(tf.keras.layers.Layer):
...
class DMFUnit(tf.keras.layers.Layer):
    # weighred add
    def __init__(self, num_in, num_out, g=1, stride=1,norm=None,dilation=None):
        super(DMFUnit, self).__init__()
        self.weight1 = tf.Variable(tf.ones(1))
        self.weight2 = tf.Variable(tf.ones(1))
        self.weight3 = tf.Variable(tf.ones(1))
...
class MFNet(tf.keras.layers.Layer): #
